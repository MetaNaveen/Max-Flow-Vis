...
function finalizeGraph () {
    /* 
        TODO: 
        Create a button in HTML (for finalizing Graph creation)
        Call this method on clicking the button 
        Get comma separated input from user for "Source and Sink" (Eg: 1,5)
        validateSourceAndSink()
        generateInputGraph()
        callAlgorithm()
    */
    ...
    var source_sink = prompt("Enter comma seperated source and sink nodes");
    if(source_sink) {
        var values = source_sink.split(',');
        var source_id = values[0];
        var sink_id = values[1];
        if(!Number.isNaN(source_id) && !Number.isNaN(sink_id)) {
            if(!source_id.includes('.') && !source_id.includes('-') && !sink_id.includes('.') && !sink_id.includes('-')) {
                s = +source_id;
                t = +sink_id;
                console.log(s,t);
            }
            else {
                alert("Please provide valid integers as source and sink ids");
                return;
            }
        }
        else {
...
function edgeClick(event) {
    ...
    var e = getEdge (id);
    if (capacity && capacity !== "0" && capacity[0] !== "-" && elem.data().capacity !== e.totalCapacity) {
        console.log(elem.data());
        e.totalCapacity = parseInt(capacity, 10);
